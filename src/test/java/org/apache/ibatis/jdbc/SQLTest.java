...
class SQLTest {
  ...
  void selectUsingLimitVariableName() {
    final String sql = new SQL() {{
      SELECT("*").FROM("com/zongze/test").ORDER_BY("id").LIMIT("#{limit}");
    }}.toString();

    assertEquals("SELECT *\nFROM com.zongze.test\nORDER BY id LIMIT #{limit}", sql);
  }

  @Test
  void selectUsingOffsetVariableName() {
    final String sql = new SQL() {{
      SELECT("*").FROM("com/zongze/test").ORDER_BY("id").OFFSET("#{offset}");
    }}.toString();
  ...
  void selectUsingLimitAndOffset() {
    final String sql = new SQL() {{
      SELECT("*").FROM("com/zongze/test").ORDER_BY("id").LIMIT(20).OFFSET(100);
    }}.toString();
  ...
  void updateUsingLimit() {
    final String sql = new SQL() {{
      UPDATE("com/zongze/test").SET("status = #{updStatus}").WHERE("status = #{status}").LIMIT(20);
    }}.toString();

    assertEquals("UPDATE com.zongze.test\nSET status = #{updStatus}\nWHERE (status = #{status}) LIMIT 20", sql);
  }

  @Test
  void deleteUsingLimit() {
    final String sql = new SQL() {{
      DELETE_FROM("com/zongze/test").WHERE("status = #{status}").LIMIT(20);
    }}.toString();

    assertEquals("DELETE FROM com.zongze.test\nWHERE (status = #{status}) LIMIT 20", sql);
  }

  @Test
  void selectUsingFetchFirstRowsOnlyVariableName() {
    final String sql = new SQL() {{
      SELECT("*").FROM("com/zongze/test").ORDER_BY("id").FETCH_FIRST_ROWS_ONLY("#{fetchFirstRows}");
    }}.toString();
  ...
  void selectUsingOffsetRowsVariableName() {
    final String sql = new SQL() {{
      SELECT("*").FROM("com/zongze/test").ORDER_BY("id").OFFSET_ROWS("#{offsetRows}");
    }}.toString();
  ...
  void selectUsingOffsetRowsAndFetchFirstRowsOnly() {
    final String sql = new SQL() {{
      SELECT("*").FROM("com/zongze/test").ORDER_BY("id").OFFSET_ROWS(100).FETCH_FIRST_ROWS_ONLY(20);
    }}.toString();
