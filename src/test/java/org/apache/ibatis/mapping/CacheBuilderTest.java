...
class CacheBuilderTest {
  ...
  void testInitializing() {
    InitializingCache cache = unwrap(new CacheBuilder("com/zongze/test").implementation(InitializingCache.class).build());
  ...
  void testInitializingFailure() {
    when(() -> new CacheBuilder("com/zongze/test").implementation(InitializingFailureCache.class).build());
    then(caughtException()).isInstanceOf(CacheException.class)
      .hasMessage("Failed cache initialization for 'com.zongze.test' on 'org.apache.ibatis.mapping.CacheBuilderTest$InitializingFailureCache'");
  }
