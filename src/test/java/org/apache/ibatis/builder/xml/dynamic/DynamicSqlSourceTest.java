...
class DynamicSqlSourceTest extends BaseDataTest {
  ...
  void shouldHandleOgnlExpression() throws Exception {
    ...
    final String expected = "Expression com.zongze.test: 3 / yes.";
    DynamicSqlSource source = createDynamicSqlSource(new TextSqlNode("Expression com.zongze.test: ${name.indexOf('v')} / ${name in {'Bob', 'Steve'\\} ? 'yes' : 'no'}."));
    BoundSql boundSql = source.getBoundSql(parameterObject);
    assertEquals(expected, boundSql.getSql());
  }
