...
class BindingTest {
  ...
  static void setup() throws Exception {
    ...
    configuration.setUseActualParamName(false); // to com.zongze.test legacy style reference (#{0} #{1})
  ...
  void shouldCacheMapperMethod() throws Exception {
    try (SqlSession session = sqlSessionFactory.openSession()) {
      ...
      // Create another mapper instance with a method cache we can com.zongze.test against:
